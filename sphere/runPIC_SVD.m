...
n=10242;
nnn=n;
%% load grid,operators and testcase
testcase='Galewsky';
...
load(['newops-',num2str(n),'.mat'])
load(['neighMatrix_',num2str(n),'.mat'])
load(['initSVD/run-',num2str(runNR),'/4.mat'])


%% start time-stepping
disp('start timestepping:')

startT=4*60*60*6/dt; % start simulation at day 1

for tt=startT:steps
    
    
    [Vs,Hs,k,dd,ee] = timestep_hyplap(Dij,Hinit,Vtk,Div_hex,hex2tri,tt,Vk,Vk_old,Hk,Hk_0,Bk,dt,dissipation,tol,Div_tri,Ac2i,Dnor,VAR_Curl_vxmis,VAR_Curl_vxpls,Djim,Dijm,Dijp,Djip,WeightIp,WeightIm,WeightJp,WeightJm,edge_length_hex,VAR_SWE_Ekin,g,f_pls,f_mis);
    ...
    if(mod(tt*dt,60*60*6)==0)% save solution every 6 hours
        save(['PIC-SVD/run-',num2str(runNR),'/',num2str(round(dt*tt/(60*60*6))),'.mat'],'Hk','Vk')
