...
for edge = 1:nedges    
    ...
    V=P*(inv(A)*[0;v(edge);u(edge)]); % project velocity field to the tangent plane of the sphere.
    VV(edge,:)=V;
    Vk(edge) = dot(V,[nor_x(edge),nor_y(edge),nor_z(edge)]); % project velocity to the normal direction of the edge. 
