function runstochasticPOD(runNR)
...
for tt=1:steps
    ...
    [Vs,Hs,k,dd,ee] = timestep_stochastic_POD(U,S,c_xx_yy_zz,c_xy_yz,c_xz,Dij,Hinit,Vtk,Div_hex,hex2tri,tt,Vk,Vk_old,Hk,Hk_0,Bk,dt,dissipation,tol,Div_tri,Ac2i,Dnor,VAR_Curl_vxmis,VAR_Curl_vxpls,Djim,Dijm,Dijp,Djip,WeightIp,WeightIm,WeightJp,WeightJm,edge_length_hex,VAR_SWE_Ekin,g,f_pls,f_mis);
    ...
    if(mod(tt*dt,60*60*6)==0)% save solution every 6 hours
        save(['POD/run-',num2str(runNR),'/',num2str(round(dt*tt/(60*60*6))),'.mat'],'Hk','Vk')
