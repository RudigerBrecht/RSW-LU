...
load(['newops-',num2str(n),'.mat'])
load(['neighMatrix_',num2str(n),'.mat'])
load(['initPOD/run-',num2str(runNR),'/4.mat'])
...
for tt=startT:steps
    ...
    if(mod(tt*dt,60*60*6)==0)        
        save(['PIC-POD/run-',num2str(runNR),'/',num2str(round(dt*tt/(60*60*6))),'.mat'],'Hk','Vk')
    end
